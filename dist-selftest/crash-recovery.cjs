...
// scripts/crash-recovery.ts
var RESULT_FILE = import_node_path3.default.join(process.cwd(), "crash-recovery-result.json");
var sleep = (ms) => new Promise((r) => setTimeout(r, ms));
var phase = process.argv[2] || "start";
function writeResult(data) {
  import_node_fs3.default.writeFileSync(RESULT_FILE, JSON.stringify(data, null, 2), "utf8");
  ...
  try {
    if (phase === "start") {
      ...
      writeResult({
        ...
        message: "started, waited 2s, then crashed (process.exit)",
        activeElapsedAtCrash: timer.getSnapshot().activeElapsedMs
      });
      timer.persistSnapshot?.();
      closeDatabase();
      import_electron3.app.exit(137);
      ...
      writeResult({
        ...
        activeOk: snap.activeElapsedMs >= 1500,
        // 至少 1.5s
      ...
      writeResult({
        ...
        noNegative: finalSnap.activeElapsedMs >= 0 && finalSnap.pauseElapsedMs >= 0 && finalSnap.wallElapsedMs >= 0,
        wallGeActive: finalSnap.wallElapsedMs >= finalSnap.activeElapsedMs,
        segmentsCount: segments.length,
        pausesCount: pauses.length,
        sessionStatus: session?.status,
        success: snap.state === "paused" && finalSnap.pauseElapsedMs >= 700 && finalSnap.activeElapsedMs >= 0
      });
      closeDatabase();
