...
export function MiniWindow() {
  ...
  const [collapsed, setCollapsed] = useState(() => window.innerWidth < 220);
  ...
  useEffect(() => {
    const syncModeFromViewport = () => setCollapsed(window.innerWidth < 220);
    window.addEventListener('resize', syncModeFromViewport);
    return () => window.removeEventListener('resize', syncModeFromViewport);
  }, []);
  ...
  return (
    <div
      ...
      className={`mini-window-shell mini-window-${state} ${collapsed ? 'mini-window-collapsed' : 'mini-window-expanded'} h-full w-full text-fg`}
      ...
      <AnimatePresence mode="wait" initial={false}>
        {collapsed ? (
          <motion.section
            ...
            transition={panelMotion}
          >
            <span className="mini-edge-progress" aria-hidden="true" />
