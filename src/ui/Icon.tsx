...
type IconTone =
  'default' | 'muted' | 'subtle' | 'accent' | 'success' | 'pause' | 'warning' | 'danger' | 'info';
...
export function createIcon<P extends Record<string, unknown>>(LucideComp: React.ComponentType<P>) {
  const Wrapped = React.forwardRef<SVGSVGElement, IconProps>(
    ({ size = 'md', tone, className = '', hover, spin, ...rest }, ref) => {
      ...
      const finalClass = [toneCls, motionCls, spinCls, className].filter(Boolean).join(' ');
      const mergedProps = {
        size: px,
        strokeWidth: stroke,
        className: finalClass,
      } as unknown as P;
      return <LucideComp ref={ref as any} {...mergedProps} {...(rest as any)} />;
    },
  );
...
export function Spinner({
  ...
  tone,
  className = '',
}: Pick<IconProps, 'size' | 'tone' | 'className'>) {
