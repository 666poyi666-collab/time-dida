 ...
 * 总历时 = 从本次 Session 开始到现在经过的自然时间
 */
export function getWallElapsedMs(snapshot: TimerSnapshot | null, now = Date.now()): number {
  if (!snapshot) return 0;
  if (snapshot.state === 'running' || snapshot.state === 'paused') {
    const starts = [
      ...(snapshot.segments ?? []).map((segment) => segment.startedAt),
      ...(snapshot.pauseEvents ?? []).map((event) => event.pauseStartedAt),
    ].filter((value) => Number.isFinite(value) && value > 0);
    if (starts.length > 0) {
      return Math.max(snapshot.wallElapsedMs, now - Math.min(...starts));
    }
  }
