...
- TimerPanel 主计时卡新增任务上下文行：运行中显示当前片段任务或本次默认任务，待开始/结束后显示即将专注任务。
- 主操作按钮从统一蓝色改为状态语义色：开始/继续/新专注为绿色，暂停为橙色，结束仍为次级按钮。
- 修复 `finished` 状态点击“开始专注”无效的问题：前端会先调用 reset，再启动新专注；带预选任务时会 reset 后原子 `startWithTask`。
- 修复 finished 状态下预选任务被 effect 立即清空的问题。
- SegmentTimeline 的专注片段颜色从结构蓝改为绿色，暂停片段继续使用橙色，滴答关联标记保持蓝色。

## v0.2.27 (2026-07-05)

### Notion 风格 tokens 落地 + 图标/托盘重构

- 全局视觉 tokens 从冷色薄荷仪表盘继续收敛为 Notion 式暖纸面：`#f6f5f4` 背景、白色纸卡、细灰边框、克制阴影。
- 结构主色改为 Notion 蓝；专注状态使用绿色，暂停状态使用橙色，避免按钮/导航和计时状态语义混用。
- 重写 `scripts/make-minimal-icons.ps1`，不再使用写死路径，生成应用图标、托盘图标和 favicon。
- 新版图标采用“纸面卡片 + 焦点环 + 任务节点 + 小色块刻度”母题，主窗口 `BrandMark` 同步改为自绘同源标识。
- 托盘图标从静态 ico 改为运行时按计时状态生成 SVG，小尺寸下也能区分未开始、专注中、暂停、已结束。

## v0.2.26 (2026-07-05)

### dida 同步真实可见 + 历史状态口径 + 小窗打磨

- dida CLI 的稳定同步路径改为优先写任务评论，确保同步记录能在滴答任务云端侧看见；任务不存在或无法读取时明确失败，不再把队列误标为已同步。
- 保留 `[FocusLink:segment:<id>]` marker 去重策略；已用临时 dida 任务验证评论写入、重复写入跳过和临时任务清理。
- 历史记录折叠行加载 session 片段摘要，不再把“没有默认任务”误显示成“未关联”；有滴答片段时显示“已关联滴答 N 段”。
- 同步状态统一为“已同步 / 未同步 / 同步失败”，清理“可同步 / 已入队 / 待同步”等模糊状态词。
- 专注小窗继续固定为 260×88 和 420×184 两态，优化透明圆角、统计卡密度、展开层级和控制区间距。
- README、前端交接、后端说明、共享契约、版本常量和打包目录同步到 `0.2.26 / release-v0226`。

## v0.2.10 (2026-07-01)

### 同步改为评论优先 + 历史状态文案校准 + 项目规范

- dida CLI 同步从“任务内容/备注优先”改为“任务评论优先”：每个专注片段写一条任务评论，评论失败时才回退写任务内容。
- 每条同步记录带 `FocusLink:segment` marker；重复点击同步时会先读取任务评论，已存在同一 marker 就跳过，避免云端重复记录。
- 历史折叠条不再只看 session 默认任务；展开后会按实际 segment 关联显示“已关联滴答 N 段 / 片段未关联”，修复折叠未关联、展开已关联的矛盾。
- 历史状态去掉“滴答可同步”这类模糊词，改为“滴答已同步 / 滴答未同步 / 无滴答片段”。
- 同步按钮和 Toast 改为“同步到滴答评论”，与实际云端写入位置一致。
- 新增 `AGENTS.md`，规范后续 AI/开发者接手项目时的目录边界、同步语义、UI 状态用词和发布流程。
