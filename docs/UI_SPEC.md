...
| 模式 | 内容 | 尺寸 |
| --- | --- | --- |
| 展开（默认） | 状态点 + 标题 + 大计时 + 当前任务 + 进度条 + 暂停/结束 + 主窗口/收起按钮 | 300×132，可拖到 240×88~520×240 |
| 紧凑（宽<260） | 单行：状态点 + 计时 + 暂停/结束/主窗口 | 流式布局，按钮始终可见 |
| 收起 | 单行横条：状态点 + 计时 + 状态文字 + 进度条 + 展开按钮 | 高度 40px |

### v0.1.5 关键约束（必须遵守）

- 贴边自动收纳（`edgeAutoCollapse` / `hoverToExpand` / `autoCollapseOnFocusStart`）**已默认关闭**，现有实现**不要沿用**（会乱跳/乱缩放/挡屏幕）
- 收起/展开在主进程已改为**直接 `setBounds` 无动画**；如需过渡动画，在渲染层用 CSS/transform 实现，**不要在主进程做 incremental setBounds 动画**
...
```ts
{
  ...
  minWidth: 240, minHeight: 88, maxWidth: 520, maxHeight: 240,
...
- 拖动调整大小、拖动移动位置（`-webkit-app-region: drag`）
...
小窗：最小 240×88，默认 300×132，最大 520×240，收起高 40px
断点：< 260px 宽（小窗紧凑）/ < 360px 宽（小窗极简，只显示时间）/ < 720px 高（主窗口隐藏 Segment 时间线）
```
