...
public class FocusGuardRootProtocolTest {
    ...
    @Test
    public void rejectsRotationThatReusesTheCurrentRoot() {
        FocusGuardRootProtocol.ProvisioningResult first = FocusGuardRootProtocol.provision(
            "account-stage-b", 1, bytes(32, 0), bytes(32, 32), bytes(12, 64), CREATED_AT
        );
        try {
            FocusGuardRootProtocol.rotate(
                first.material,
                bytes(32, 32),
                first.material.rootKey,
                bytes(12, 80),
                bytes(12, 64),
                CREATED_AT + 1
            );
            throw new AssertionError("same-root rotation must fail");
        } catch (IllegalStateException expected) {
            assertEquals("Focus Guard root envelope verification failed", expected.getMessage());
        }
    }
    ...
    @Test
    public void rejectsAppliedRotationThatReusesTheCurrentRoot() throws Exception {
        FocusGuardRootProtocol.ProvisioningResult first = FocusGuardRootProtocol.provision(
            "account-stage-b", 1, bytes(32, 0), bytes(32, 32), bytes(12, 64), CREATED_AT
        );
        FocusGuardRootProtocol.RootMaterial sameRootNextGeneration = FocusGuardRootProtocol.provision(
            "account-stage-b",
            2,
            first.material.rootKey,
            bytes(32, 32),
        ...
        Method wrapRoot = FocusGuardRootProtocol.class.getDeclaredMethod(
            "wrapRoot",
            String.class,
            String.class,
            byte[].class,
            FocusGuardRootProtocol.RootMaterial.class,
            Integer.class,
            byte[].class
        );
        wrapRoot.setAccessible(true);
        String craftedEnvelope = (String) wrapRoot.invoke(
            null,
            "rotation",
            "direct-root",
            first.material.rootKey,
            sameRootNextGeneration,
            Integer.valueOf(1),
            bytes(12, 80)
        );

        try {
            FocusGuardRootProtocol.applyRotation(craftedEnvelope, first.material);
            throw new AssertionError("applied same-root rotation must fail");
        } catch (IllegalStateException expected) {
            assertEquals("Focus Guard root envelope verification failed", expected.getMessage());
        }
    }
