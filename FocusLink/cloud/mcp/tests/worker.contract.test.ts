...
const CALLER_DEVICE_TOKEN =
  "fl2_account1_caller01_0123456789abcdefghijklmnopqrstuvwxyzABCDE";
...
describe("Worker canonical HTTP contract", () => {
  ...
  it("reports ready only when D1, service binding, paired reader, and OAuth RS are configured", async () => {
    ...
    expect(
      oauth.calls.some((call) =>
        call.url.endsWith("/.well-known/oauth-authorization-server"),
      ),
    ).toBe(true);
    expect(oauth.calls.some((call) => call.url.endsWith("/jwks.json"))).toBe(
      true,
    );
    expect(oauth.calls.some((call) => call.url.endsWith("/introspect"))).toBe(
      true,
    );
  });
  ...
  it("accepts an active audience-bound OAuth token and serves direct Account DO records", async () => {
    ...
    expect(
      oauth.calls.filter((call) => call.url.endsWith("/introspect")),
    ).toHaveLength(4);
    expect(
      oauth.calls.filter((call) => call.url.endsWith("/jwks.json")),
    ).toHaveLength(1);
    expect(
      oauth.calls.filter((call) => call.url.endsWith("/introspect")),
    ).toEqual(
  ...
  it("routes the client canonical v2 live surface through the service binding", async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.headers.get("x-focuslink-authority")).toBe(
      "durable-object-v2",
    );
    expect(await response.json()).toEqual({
      protocolVersion: 1,
  ...
  it("requires exact AS service actions for owner device inventory and revocation", async () => {
    const unauthenticated = await SELF.fetch(
      `${CANONICAL}/sync/v1/pair/devices`,
    );
...
async function callTool<T = ToolPayload>(
  ...
  const content = (
    message.result as { content?: Array<{ type?: string; text?: string }> }
  )?.content;
