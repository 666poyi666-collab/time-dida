...
const READER_TOKEN = 'fl2_account1_reader01_0123456789abcdefghijklmnopqrstuvwxyzABCDE';
const NONCE = 'n'.repeat(43);

describe('canonical one-time device pairing', () => {
  it('allows the trusted-device Authorization header through browser preflight', async () => {
    const response = await handleCanonicalPairing(
      new Request('https://worker.test/sync/v1/pair/offers', {
        method: 'OPTIONS',
        headers: {
      ...
      pairEnv(binding(async () => Response.json({ error: 'must_not_run' }))),
      false,
    );
  ...
  it('uses an edge-verified owner capability and a distinct private authority credential upstream', async () => {
    const upstream = binding(async (request) => {
      ...
      expect(request.headers.get('x-focuslink-pair-authority')).toBe(PAIR_AUTHORITY_TOKEN);
  ...
  it('rejects OAuth, device, and OOB credentials on the anonymous claim surface', async () => {
    ...
    const credentialHeaders: Array<Record<string, string>> = [
      ...
      {
        'x-focuslink-service-credential': env.FOCUSLINK_PAIR_SERVICE_CREDENTIAL!,
      },
  ...
  it('lists and revokes devices only after the edge verifies the owner service', async () => {
    ...
    const revoke = await handleCanonicalPairing(
      new Request('https://worker.test/sync/v1/pair/devices/device-reader01/revoke', {
        method: 'POST',
      }),
      env,
      true,
        ...
        devices: [
          {
            ...
            scopes: ['sync:read', 'live:read', 'devices:manage', 'backups:manage'],
  ...
  it.each([
    ...
    [
      'more than 16 scopes',
      {
        scopes: ['sync:read', ...Array.from({ length: 16 }, (_, index) => `feature${index}:read`)],
      },
    ],
...
function pairEnv(upstream: { fetch: ReturnType<typeof vi.fn> }): PairingEnv {
  return {
    ...
    FOCUSLINK_PAIR_SERVICE_CREDENTIAL: 'fls_0123456789abcdefghijklmnopqrstuvwxyzABCDEFG',
    FOCUSLINK_PAIRING_ENABLED: 'true',
    FOCUSLINK_DEVICE_TOKEN: 'fl2_account1_project1_0123456789abcdefghijklmnopqrstuvwxyzABCDE',
...
function pairRequest(path: string, value: unknown, authorization?: string): Request {
