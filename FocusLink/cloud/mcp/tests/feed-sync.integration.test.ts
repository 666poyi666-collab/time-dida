...
describe("FocusLink v2 authoritative feed adapter", () => {
  ...
  it("commits each page atomically, marks interruption degraded, and resumes from the committed cursor", async () => {
    ...
    await expect(
      syncAuthoritativeFeed(feedEnv, { fetcher: upstream.fetch }),
  ...
  it("clears the old projection and replays from null when the epoch changes", async () => {
    ...
    expect((await readProjection(env.DB, TEST_ACCOUNT_KEY)).sessions.map((session) => session.id)).toEqual([
      "new-session",
    ]);
    expect((await listFeedEntities(env.DB, TEST_ACCOUNT_KEY)).some((row) => row.entity_id === "old-session"))
      .toBe(false);
  });
