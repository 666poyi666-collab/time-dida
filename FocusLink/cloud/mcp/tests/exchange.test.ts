...
describe("canonical /sync/v2 adapter", () => {
  it("proxies status and exchange only through the configured service binding", async () => {
    ...
    const status = await handleCanonicalSync(
      request("/sync/v2/status", "GET", TOKEN),
      env,
    );
    ...
    expect(exchange.headers.get("x-focuslink-authority")).toBe(
      "durable-object-v2",
    );
  ...
  it("rejects malformed credentials, JSON, oversized/unknown fields, and denied origins", async () => {
    ...
    expect(
      (
        await handleCanonicalSync(
          request("/sync/v2/exchange", "POST", TOKEN, extra),
          env,
        )
      ).status,
    ).toBe(400);
  ...
  it("proxies the canonical task and live surfaces through the private authority", async () => {
    ...
    expect(
      (await handleCanonicalSync(request("/sync/v2/tasks", "GET", TOKEN), env))
        .status,
    ).toBe(200);
    expect(
      (
    ...
    expect(
      (
        await handleCanonicalSync(
          request(
            "/sync/v2/live/wait?afterRevision=3&waitMs=1000",
            "GET",
            TOKEN,
          ),
          env,
    ...
    expect(
      (
        await handleCanonicalSync(
          request("/sync/v2/live/command", "GET", TOKEN),
          env,
        )
      ).status,
    ).toBe(405);
  ...
  it("proxies device inventory and exact targeted revocation without accepting a body", async () => {
    ...
    expect(
      (
        await handleCanonicalSync(
          request("/sync/v2/devices", "GET", TOKEN),
          env,
        )
      ).status,
    ).toBe(200);
    ...
    expect(
      (
        await handleCanonicalSync(
          request("/sync/v2/devices?include=secrets", "GET", TOKEN),
          env,
        )
      ).status,
    ).toBe(400);
  ...
  it("rejects upstream redirects and unexpected status queries", async () => {
    ...
    const env = exchangeEnv(upstream);
    const redirected = await handleCanonicalSync(
      request("/sync/v2/live", "GET", TOKEN),
      env,
    );
    ...
    const queried = await handleCanonicalSync(
      request("/sync/v2/status?tail=1", "GET", TOKEN),
      env,
    );
  ...
  it("retires v1 data aliases and does not preflight unknown v2 routes", async () => {
    ...
    const unknown = await handleCanonicalSync(
      request("/sync/v2/not-real", "OPTIONS", TOKEN),
      env,
    );
    expect(unknown.status).toBe(404);

    const known = await handleCanonicalSync(
      request("/sync/v2/live", "OPTIONS", TOKEN),
      env,
    );
    expect(known.status).toBe(204);
    expect(known.headers.get("access-control-allow-methods")).toBe(
      "GET, OPTIONS",
    );
    expect(upstream.fetch).not.toHaveBeenCalled();
  });
  ...
  it("enforces exact live-wait queries and per-route body limits", async () => {
    ...
    for (const path of invalidWaits) {
      expect(
        (await handleCanonicalSync(request(path, "GET", TOKEN), env)).status,
      ).toBe(400);
    }
    expect(
      (
        await handleCanonicalSync(
          request("/sync/v2/tasks?ignored=true", "GET", TOKEN),
          env,
        )
      ).status,
    ).toBe(400);
