...
export async function handleCanonicalSync(
  ...
  if (
    url.pathname === "/sync/v1/status" ||
    url.pathname === "/sync/v1/exchange"
  ) {
  ...
  if (request.method === "OPTIONS")
    return preflight(request, env, route.methods);
  ...
  if (!query.ok)
    return withCors(request, env, syncError(query.status, query.code));
  if (!env.FOCUSLINK_UPSTREAM) {
    return withCors(
      request,
      env,
      syncError(503, "upstream_service_binding_missing"),
    );
  }

  const token = deviceBearerToken(request);
  if (!token)
    return withCors(request, env, syncError(401, "device_credential_required"));
  if (
    (env.OAUTH_RS_CLIENT_SECRET &&
      constantTimeEqual(token.value, env.OAUTH_RS_CLIENT_SECRET)) ||
    (env.FOCUSLINK_DEVICE_TOKEN &&
      constantTimeEqual(token.value, env.FOCUSLINK_DEVICE_TOKEN)) ||
    ...
    return withCors(
      request,
      env,
      syncError(403, "credential_boundary_violation"),
    );
  }
    ...
    if (route.maxBodyBytes === 0) {
      try {
        ...
        return withCors(
          request,
          env,
          syncError(400, "request_body_unreadable"),
        );
      }
      init = { method: "POST" };
    } else {
...
function canonicalRoute(pathname: string): CanonicalRoute | null {
  if (
    /^\/sync\/v2\/devices\/device-[A-Za-z0-9-]{6,194}\/revoke$/.test(pathname)
  ) {
    return {
      methods: ["POST"],
      query: "none",
...
function validateCanonicalQuery(
  ...
  const afterRevision = strictUnsignedInteger(
  ...
  return {
    ...
    timeoutMs: Math.max(
      UPSTREAM_TIMEOUT_MS,
      waitMs + LIVE_WAIT_TIMEOUT_MARGIN_MS,
    ),
  };
}
...
function validateExchangeBody(value: unknown, devicePublicId: string): boolean {
  ...
  return value.mutations.every((mutation) =>
    validateMutation(
      ...
      value.accountGeneration as number,
    ),
  );
}
...
function validateMutation(
  ...
  if (
    ...
    !FEED_ENTITY_TYPES.includes(
      value.entityType as (typeof FEED_ENTITY_TYPES)[number],
    ) ||
    ...
    return false;
  if (
    (value.kind === "put" || value.kind === "restore") &&
    !isRecord(value.payload)
  )
    return false;
  if (
    (value.kind === "delete" || value.kind === "purge") &&
    value.payload !== null
  )
    return false;
  return true;
...
function deviceBearerToken(
  ...
  const header = /^Bearer ([^\s]{1,4096})$/i.exec(
    request.headers.get("authorization") ?? "",
  );
  if (!header) return null;
  const match = DEVICE_TOKEN_PATTERN.exec(header[1]);
  return match
    ? { value: header[1], accountPublicId: match[1], devicePublicId: match[2] }
    : null;
}
...
function syncError(status: number, code: string): Response {
  return new Response(JSON.stringify({ error: { code, message: code } }), {
    ...
    headers: syncHeaders(
      status === 401
        ? { "www-authenticate": 'Bearer realm="focuslink-device-sync"' }
        : {},
    ),
  });
}
