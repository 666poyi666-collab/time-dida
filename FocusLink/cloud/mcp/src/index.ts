...
const OAUTH_READ_SECURITY_SCHEMES = [
  { type: "oauth2", scopes: ["focuslink:read"] },
] as const;
...
function createFoxlinkMcpServer(env: Env): McpServer {
    ...
    const listFocusRecords = async ({ limit }: { limit: number }) =>
      focusRecordsResponse(env, limit);
    ...
    server.registerTool(
      ...
      {
        description:
          "读取 FocusLink Account DO 的当前专注状态和最近完成记录。",
        inputSchema: {},
    ...
    server.registerTool(
      ...
      {
        description:
          "兼容旧名称：读取 FocusLink 权威 v2 feed 的同步状态和最近完成记录。",
        inputSchema: {},
        annotations: READ_ONLY_TOOL_ANNOTATIONS,
        _meta: OAUTH_READ_TOOL_META,
      },
    ...
    server.registerTool(
      ...
      {
        description:
          "列出 Account DO 权威专注记录，包含每段专注和暂停时间线。",
        inputSchema: {
    ...
    server.registerTool(
      ...
      {
        description:
          "按具体任务聚合 FocusLink 专注次数、完成/中止次数、有效时长和最近专注时间。",
        inputSchema: {
    ...
    server.registerTool(
      ...
      {
        description:
          "读取权威 feed checkpoint、epoch、freshness 和实体/tombstone 计数。",
        inputSchema: {},
        annotations: READ_ONLY_TOOL_ANNOTATIONS,
        _meta: OAUTH_READ_TOOL_META,
      },
...
async function runFeedSync(env: Env): Promise<SyncAttempt> {
  try {
    ...
    const adapterError =
      error instanceof FeedAdapterError
        ? error
        : new FeedAdapterError("feed_projection_failed");
    return {
      ok: false,
...
async function requestFeedSync(env: Env): Promise<SyncAttempt> {
  try {
    const id = env.FOCUSLINK_FEED_SYNC.idFromName(
      env.FOCUSLINK_ACCOUNT_KEY || "unconfigured",
    );
    const response = await env.FOCUSLINK_FEED_SYNC.get(id).fetch(
      "https://feed.internal/sync",
      {
        method: "POST",
      },
    );
...
function authorityMetadata(
  ...
  let freshness:
    "current" | "degraded" | "stale" | "incomplete" | "never_synced";
...
export const worker = {
  async fetch(
    ...
    if (
      ...
      /^\/sync\/v1\/pair\/devices\/device-[A-Za-z0-9-]{6,194}\/revoke$/.test(
        url.pathname,
      )
    ) {
    ...
    if (
      url.pathname.startsWith("/sync/v1/") ||
      url.pathname.startsWith("/sync/v2/")
    ) {
    ...
    if (url.pathname === "/mcp") {
      ...
      const authentication = await authenticateMcpRequest(
        request,
        env,
        metadataUrl,
      );
      if (!authentication.ok) {
        return (
          (await mcpToolOAuthChallenge(request, authentication.response)) ??
      ...
      const grantedScopes = new Set(
        authentication.claims.scope.split(/\s+/).filter(Boolean),
      );
...
async function readiness(env: Env): Promise<Response> {
  ...
  if (
    !/^fls_[A-Za-z0-9_-]{43,160}$/.test(
      env.FOCUSLINK_PAIR_SERVICE_CREDENTIAL ?? "",
    ) ||
    !/^[A-Za-z0-9._~-]{3,200}$/.test(
      env.FOCUSLINK_PAIR_SERVICE_CLIENT_ID ?? "",
    ) ||
  ...
  return json(
    {
      ...
      service: "foxlink-cloud-mcp",
      authority: FEED_AUTHORITY,
      storage: failures.includes("d1_projection_schema")
        ? "unavailable"
        : "ready",
...
function safeOAuthProbeFailure(error: unknown): string {
  ...
  return /^oauth_(?:metadata|jwks|introspection)_(?:unavailable|invalid)$/.test(
    code,
  )
    ? code
    : "oauth_dependencies";
...
async function authorizePairService(
  request: Request,
  env: Env,
): Promise<Response | null> {
  if (request.method !== "POST") return methodNotAllowed("POST, OPTIONS");
  if (!env.PAIR_RATE_LIMITER)
    return json({ error: "pair_rate_limiter_not_configured" }, 503);
  ...
  if (
    ...
    request.headers.get("x-focuslink-service-client") !==
      env.FOCUSLINK_PAIR_SERVICE_CLIENT_ID ||
    request.headers.get("x-focuslink-service-audience") !== expectedAudience ||
    request.headers.get("x-focuslink-service-action") !==
      "focuslink.pair.offer.create"
  ) {
...
async function authorizeBootstrapAdminService(
  ...
  if (!authorization) {
    ...
    response.headers.set(
      "www-authenticate",
      'FocusLinkService realm="focuslink-bootstrap-admin"',
    );
    return response;
  }
  if (
