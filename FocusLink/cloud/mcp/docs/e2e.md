...
7. With a token carrying `focuslink:read focuslink:write`, call `focuslink_get_current_time`, `focuslink_list_projects`, `focuslink_get_project` and `focuslink_list_tasks`. Verify the requested timezone, local day boundaries and current revision. Create a temporary FocusLink project, parent task and child task carrying start/due timestamps, priority, tags and a bounded weekly recurrence. Complete once and verify the next occurrence/date/count rather than a completed task; exhaust the count, restore the final occurrence, then update and move the subtree while supplying the returned `revision` as `expectedRevision`. Each successful response must be a redacted confirmation from Account DO.
...
13. On phone and tablet create/update/finish focus records, then verify the cloud summary's concrete task, count and duration. Repeat after app restart and network recovery. OPPO/watch is retired and excluded from new acceptance.
