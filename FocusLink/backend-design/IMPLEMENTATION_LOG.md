...
- **ChatGPT Web 现状**：设置中已有 `FocusLink（云端 OAuth）`，URL 指向正式 MCP。2026-08-30 已执行插件定义刷新，页面现在列出 `focuslink_get_current_time`、清单/任务读写与 `focuslink:write` scope；连接时间仍保留旧记录，现有 OAuth 授权尚未扩权。页面停在“重新连接”前，按动作时确认要求等待用户明确允许；生产任务可逆闭环仍未执行，不能写成已配置完成。
