...
- **packaged 候选状态**：提交 `59a12f3` 的 `npm run dist` 产物已通过 packaged UI 与修复后的 mini smoke；由于 mini smoke 基础设施随后形成提交 `2d33177`，最终发行产物必须从包含该提交及本日志的后续干净提交重新 `dist`，再复跑 UI/mini/live-fallback，不能复用较早 commit 身份。当前 `adb devices -l` 无在线设备，Windows/小米/华为同版实装仍未发生。
