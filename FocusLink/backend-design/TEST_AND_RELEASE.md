...
npm run smoke:dida:ui -- ../release-v0113/win-unpacked/FocusLink.exe
```
...
目录规则：`0.11.3 → release-v0113`，`0.2.10 → release-v0210`。发布目录位于源码工作区父级；仓库本地只保留最新三个 release 目录，更老的安装包由 GitHub Releases 长期保存。
...
2. 在 release-record commit 上创建并推送与版本完全一致的 annotated tag，例如 `v0.11.3`。
3. 创建 GitHub Release，标题使用 `FocusLink v0.11.3`，正文使用本地 `RELEASE_NOTES.md`。
