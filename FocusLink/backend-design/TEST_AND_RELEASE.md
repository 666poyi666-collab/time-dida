...
> 这是发布门禁，不是建议清单。补丁尾号为 `0` 或 `5` 的 GitHub 上传节点只有在本页全部满足并创建 GitHub Release 后才算发布完成；其余补丁按本地迭代门禁完成。
>
...
GitHub 采用五版本集中上传节奏：仅补丁尾号为 `0` 或 `5` 的版本推送 `main`、创建 annotated tag 和 GitHub Release。其他补丁版本只在本地完成测试、三端同版安装、CHANGELOG/实施日志、四文件发布目录和 Android APK 备份，不创建或推送 tag，也不上传 GitHub；下一个上传节点汇总这些本地版本的变更。
