...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| FL-REQ-20260730-CLOUD-MCP | 2026-07-30 | 电脑关闭时手机和平板直接从云端获取、控制和上传专注；ChatGPT 通过云端只读 MCP 获取当天完整记录与 live；不接 PersonalMcpGateway，不增加 E2EE、R2 或手表功能 | 不再运行 ADB reverse/内嵌同步中继；重启后同一账本只出现一次且重复同步不增冲突 | App 打开在线即拉取并长轮询；离线完成账本补传，已有不同 cloud live 时保留 fork conflict | 与手机相同，直接连接同一 Account DO authority | 自动化已通过；待云部署与实机 PC-off 验收 | v0.12.70 | `deviceSyncV2Service`、`v2OutboxStore`、Account DO records DTO、cloud MCP、OAuth FocusLink-only rotation |
