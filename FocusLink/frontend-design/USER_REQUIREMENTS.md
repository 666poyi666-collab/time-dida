...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| FL-REQ-20260730-ACCOUNT-SYNC | 2026-07-30 | 唯一管理员账号登录后自动获得实时、任务与账本同步；普通用户不再填写服务地址、令牌或配对码 | 旧 `fl2` 原位识别登录；只显示账号/状态/立即同步/退出；新登录后台登记独立 Windows device | 登录按钮跳系统 owner 流；成功后自动登记 phone device 并写 Keystore；无地址/令牌/配对 UI | 平板同手机；OPPO 手表只从手机确认并获得独立 watch device | 客户端/私有 authority 与严格 start/poll 合同已完成；旧凭据继续同步；公网 bootstrap 与手表 companion 确认待独立部署授权 | v0.12.73 | `deviceSyncAccountService`、`accountBootstrapProtocol`、canonical probe、Account DO register、桌面/移动设置与手表登录态 |
| FL-REQ-20260730-TASK-FRESHNESS | 2026-07-30 | PC 发布的新任务快照必须在手机/平板前台自动收敛，旧 revision 或缓存响应不能覆盖新快照 | 发布 pending 只有 authority 回读同 device/payload 后清除 | 可见态每 15 秒 `no-store` 拉取；revision 只前进；同 revision 异文拒绝 | 同手机，父子 ID/parentId 数量保持 | 协议与无私人正文回归已完成，待 0.12.73 四端实装 | v0.12.73 | `taskSnapshotProtocol`、`deviceSyncService`、`MobileApp` freshness effect、`taskSnapshotFreshness.test.ts` |
| FL-REQ-20260730-DESKTOP-START | 2026-07-30 | 修复 Windows“开始专注”被实时 403 阻断、设置仪表预览显示不全、时间之带再次出现毛虫边缘；用户发现的问题必须进入需求表、实施日志、CHANGELOG 和可失败的回归门禁 | `fl2` 绑定 ID 贯穿 live/task 写入；空闲 401/403 可本地开始；九仪表预览不裁切；专注绿带全状态无锯齿/浮尘；日志不得只写 `[object Object]` | 同版安装并回读；移动功能不因桌面降级语义改变 | 同版安装并回读；华为 capsule 保持 | 实施中，待四端实装 | v0.12.72 | `deviceSyncService`、`FocusTimerController`、`TemporalRibbon`、仪表 preview CSS、live/device/UI smoke、`SYNC_TROUBLESHOOTING` |
| FL-REQ-20260730-DAY-LEDGER | 2026-07-30 | 建立共享 07:00–22:00 有效日与精确空档契约；focus/pause/gap 严格守恒，旧记录只给 estimated，不持久化第二份 gap 事实 | Dashboard 显示 SVG focus/pause/gap 甜甜圈、24h 轴、精确空档列表；无 focus 显示“今日尚未启动”；玻璃只用于控制层 | 只消费共享 `dayLedgers`，不得在移动 renderer 重写 gap 算法 | 同手机；宽屏布局保持唯一账本与任务投入 | 桌面/共享已实现，待移动消费与集成验收 | v0.12.73 | `shared/dayLedgerAnalytics.ts`、`SessionAnalyticsResult.dayLedgers`、`HistoryInsights`、纯函数/renderer/仪表回归 |
| FL-REQ-20260730-MOBILE-UI | 2026-07-30 | 重做手机和平板 renderer，建立深墨框架、连续时间仪器工作面和真正分开的手机/平板响应式层级；保留 Windows 两态小窗、华为 capsule、小米系统表面与手表路径 | 桌面产品 UI 不改；须覆盖安装并回读同版，复验两态 mini | 393×852 首屏可见主读数、任务输入、时间之带与粘底主操作；深浅主题均无横向溢出 | 640px 竖屏不再硬塞上下文侧栏，760px 起才展开双栏；华为 capsule 模块保持 | 待四端实装 | v0.12.72 | `mobile.css`、`MobileApp`、`AppNavigation`、`FocusConsole`、响应式合同测试、`FRONTEND_SPEC` §10 |
