...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| FL-REQ-20260730-ACCOUNT-SYNC | 2026-07-30 | 唯一管理员账号登录后自动获得实时、任务与账本同步；普通用户不再填写服务地址、令牌或配对码 | 旧 `fl2` 原位识别登录；只显示账号/状态/立即同步/退出；新登录后台登记独立 Windows device | 登录按钮跳系统 owner 流；成功后自动登记 phone device 并写 Keystore；无地址/令牌/配对 UI | 平板同手机；OPPO 手表只从手机确认并获得独立 watch device | 客户端与私有 authority 已实现；公网 bootstrap 待 canonical identity gateway 独立部署授权 | v0.12.72 | `deviceSyncAccountService`、`accountBootstrap`、`identityProtocol`、Account DO register、桌面/移动设置与手表登录态 |
