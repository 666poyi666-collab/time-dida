...
| FL-REQ-20260724-PAIRING | 2026-07-24 | 用一次性二维码/短码简化连接，长期凭据安全保存 | 已验证启动、周期探测和晚连接协调；两台设备独立配对后立即同步 | 小米晚连接/重连与同代幂等通过；临时已结束账本写入与 tombstone 清理均收敛 | 华为断开再连接后一个探测周期内恢复 reverse 并同步；临时账本写入与清理均收敛 | 已验收 | v0.12.43 | `/v1/pair`、`AndroidSyncCoordinator`、Android 深链、配对与协调器测试 |
| FL-REQ-20260724-TRI-END | 2026-07-24 | 每轮 UI/行为迭代递增补丁版本并完成 Windows、手机、平板同版矩阵 | 待安装并回读 0.12.43 | 小米待回读 0.12.43 / 1243 | 华为待回读 0.12.43 / 1243 | 0.12.42 标签门禁失败，0.12.43 重新执行同版门禁 | v0.12.42 起持续 | `AGENTS.md`、发布规范、前后端接手清单、实施日志 |
