...
- [ ] 番茄可上传数与超过 7 天的历史分开显示；过期历史明确为“已停止重试、保留本机”，不冒充待上传、失败或上传成功。
- [ ] 已有番茄 marker 的学科修改若外部更新失败会进入 durable queue；队列未清空前不能把旧记录的 `isSynced=1` 显示成新学科“上传已确认”。
- [ ] 设备状态将当前实时连接、账本新鲜度、最后成功时间和最近诊断分开；其他设备的 `lastSeenAt` 不冒充当前在线。
- [ ] 620px 及以上平板的字体样张和九仪表预览可完整阅读，选择控件与预览不挤压、无横向溢出。
- [ ] 任务选择、任务树和任务搜索复用现有 feature，不复制算法；parentId 树不重复、不丢项且完整显示父路径，展开、选择、开始互不触发，选择后仍停留任务页。
...
- [ ] 八套本地界面字体（Noto/文楷/新致宋/漫黑/新晰黑/得意黑/思源宋体/站酷快乐体）使用真实不同字形骨架，不以同字体粗细冒充选项。
...
- [ ] 统计按结论→focus/pause/gap 甜甜圈→四 KPI→共享全天轴/多日三段构成→精确 gap→100% 任务构成带顺读；移动 Dashboard 只消费 `shared/dayLedgerAnalytics.ts`，不复制 gap 算法，不把无 focus 的一天画成全天空档。
...
- [ ] 非交互移动 viewport smoke 覆盖 360、412、640×1024、≥760 与横屏的亮/暗主题，逐页断言 overflow 和 console；不得依赖 `?review=1` 或遗留临时 harness。
