...
- [ ] 界面蓝=操作、专注绿=运行、暂停红=损耗、深红=危险；专注色修改只映射 `--app-success*`，不扩散到导航与按钮。
...
- [ ] 沉浸模式覆盖层复用同一 TimerDial 与 canvas 时间之带：包含当前任务、状态、累计三项、全部控制与底部放大时间之带（约 26vh，最小 170px），Esc 或退出按钮离开，不复制第二套仪表实现。
- [ ] 统计单日为 24 小时织带、多日为日期×时段矩阵、单次质量为珠链、时间去向为马赛克比例带+精确占比行（含「未关联」）；全部来自真实 analytics 数据，无环形图、无 KPI 卡片墙、无假图。
