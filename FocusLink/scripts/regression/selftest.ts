...
// 用法：npm run selftest:build && npx electron dist-selftest/selftest.cjs
...
configureIsolatedUserData('selftest', true);
  ...
  try {
    ...
    result.db = {
      ...
      segmentsNonOverlapping:
        segments.length === 2 ? (segments[0].endedAt ?? 0) <= segments[1].startedAt : true,
    };
