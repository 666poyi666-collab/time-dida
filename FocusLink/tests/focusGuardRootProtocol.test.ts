...
describe('Focus Guard root protocol', () => {
  ...
  it('rejects a rotation that reuses the current root', async () => {
    ...
    const createdAt = CREATED_AT + 1;
    const aadBytes = new TextEncoder().encode(
      `focus-guard-root|rotation|${ACCOUNT}|1|2|${createdAt}`,
    );
    const wrappingKey = await crypto.subtle.importKey('raw', exactBuffer(ROOT), 'AES-GCM', false, [
      'encrypt',
    ]);
    const ciphertext = await crypto.subtle.encrypt(
      {
        name: 'AES-GCM',
        iv: exactBuffer(ROTATION_NONCE),
    ...
    const aadHash = new Uint8Array(await crypto.subtle.digest('SHA-256', exactBuffer(aadBytes)));
    await expect(
      applyFocusGuardRootRotation({
        current: first.material,
        envelope: {
          ...
          fromGeneration: 1,
          generation: 2,
          nonce: encodeBase64Url(ROTATION_NONCE),
