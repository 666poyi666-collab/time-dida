...
const builderPatchScript = readFileSync(
  resolve('scripts/build/patch-electron-builder-nsis.cjs'),
  'utf8',
);
...
describe('Windows installer process policy', () => {
  it('keeps the smoke bypass and closes current-user instances without enumeration', () => {
    ...
    expect(installerScript).toContain("TrimEnd('\\')+'\\'");
    expect(installerScript).toContain('StartsWith($$profileRoot');
