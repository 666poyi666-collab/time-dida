...
- [ ] 主工作面使用精密明亮的中性画布、高不透明表面和单一受控状态光；没有大面积 blur、径向光晕、文字发光或持续扫光。
- [ ] 主窗可读/可操作文案不小于 11px，10px 只用于非核心辅助标签；小窗当前时间保持收起 23.5px / 展开 30px。
...
- [ ] 动效不改变布局，不持续消耗 GPU，并实现 reduced-motion 退化。
...
- [ ] 边缘自动收起不在拖动中抢鼠标，展开方向不越过 work area。
...
- [ ] 在根目录 `CHANGELOG.md` 的新版本段落记录用户可见变化。
- [ ] 从 [.github/RELEASE_NOTES_TEMPLATE.md](../.github/RELEASE_NOTES_TEMPLATE.md) 生成对应 `release-v*/RELEASE_NOTES.md`。
