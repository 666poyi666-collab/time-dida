...
> 当前版本：v0.12.104
>
> 0.12.104 让每台电脑、手机和平板都能显示自己的 8 位本机码：任意一台把另一台的码输入一次，两台就进入同一同步空间并开始同步任务、专注和账本；无需账号顺序、管理员码或配对次数限制。本轮完成三端同版实装前不标记为已发布；OPPO 手表保持退役/冻结。
>
...
FocusLink 自有普通清单可在电脑、手机和平板确认后删除；任务和子任务会原子迁入不可删除的收件箱，不会静默丢失。云端 `foxlink-cloud-mcp` 也直接管理同一任务快照，支持清单/任务读取、创建、更新、完成、恢复、删除和移动，以及截止时间、优先级、标签和父子关系；写入使用 `operationId` + `expectedRevision`，冲突不覆盖。
