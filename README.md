...
> 当前源码迭代：v0.11.7
>
> 版本主题：「安静的桌面时间仪器」视觉重构（阶段 1 专注页已落地）· IBM Plex 字栈 · 深 teal 强调
...
v0.11.6 为最近正式版（本地资产见上表，线上状态以 GitHub Releases 为准）；v0.11.7 当前是源码迭代，正式资产与线上记录只能在完整门禁通过后发布到 [GitHub Releases](https://github.com/666poyi666-collab/time-dida/releases)。每个新版本都必须由 tag workflow 复验已提交产物，并回读正文、目标提交、附件名称、字节数、下载链接与下载后 SHA256；不能移动失败的旧 tag，也不能补造未完成门禁的历史 Release。
