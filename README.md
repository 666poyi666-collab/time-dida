...
- 界面提供 Noto Sans SC、霞鹜文楷、霞鹜新致宋、霞鹜漫黑四套真实不同且本地嵌入的字体；五套计时仪表继续使用各自独立字形与结构。
- 时间之带是单一 canvas：刻度即进度，近景/远景可手动选择，专注和暂停都按秒更新；状态转换使用 720ms 变焦，并在当前边界绘制随时间消逝的状态粒子。
- 统计页重构为顺读日报：结论、四项 KPI、24 小时专注/暂停时间轴、任务投入排行和最近会话表；多日范围切换为每日趋势。
...
- 小窗尺寸保持收起 `184×35`、展开 `280×84`；展开态完整显示任务、当前时间、累计专注/暂停/总历时与全部控制，收起态以 2px 进度轨和状态粒子表达时间消逝。拖拽释放后先吸附，再播放 320ms 收束过渡后折叠。
