...
## v0.12.104 - 2026-08-26（每台设备本机码与反向批准）

- 未授权设备也会自动生成自己的 8 位本机配对码；在任一已授权设备输入该码即可批准，新设备后台领取独立凭据并自动开始任务、实时专注与账本同步。
- 保留原有“新设备输入已授权设备码”的兼容流程；两种方式任选其一，不再要求用户先理解配对方向。
- 8 位码只用于批准，高强度领取凭据只保留在申请设备；服务端分别保存域分离 HMAC，不保存明文配对码或领取凭据。
- 新配对设备获得设备列表与撤销权限；删除设备只撤销该设备的同步凭据，不删除云端任务、专注记录或其他设备数据。
- 首次授权网页改为 FocusLink 中文自适应界面，并明确区分 43 位管理员授权码与应用内 8 位设备配对码。
