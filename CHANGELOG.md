...
## v0.11.6 - 2026-07-17（Aurora Ink 视觉重建 · 左轨道导航 · 红暂停）

- **前端视觉语言彻底重建为 Aurora Ink**：抛弃顶部胶囊导航，改为约 76px 左侧垂直轨道（品牌、图标+标签导航、底部计时状态胶囊）；亮色为主角——冷瓷白画布上漂移的虹彩氛围光晕、白瓷工作面配彩色 edge light、浓郁渐变的发光主操作、88–148px 超大计时排印；暗色同构、光晕更浓。
- **暂停语义色由琥珀改回红色**：状态徽章、活动轨、片段账本、历史图表与小窗暂停态统一使用 `--app-pause` 红色系及 `--shadow-glow-pause` 光晕；运行态保持绿色语义。
- **发光只落在操作与状态载体**：主按钮、激活导航与运行状态使用 `--shadow-glow-accent`；继续禁止 backdrop-filter，reduced-motion 下持续动画全部停用。
- **任务/统计/设置/小窗同步重建**：任务工作台白瓷壳 + 左轨清单导航、统计统一分析画布虹彩网格与构成环、设置白瓷段落卡与 42×24 开关契约、小窗两态材质与红色暂停信号；全部 smoke DOM 钩子与文案语义保持不变。
- **Geist「锐界」为默认字体**：Manrope「舒展」保留可选；应用与托盘图标重建为虹彩 BrandMark 几何；键盘焦点环仅在 Tab 进入键盘导航后显示。
- **回归与文档同步**：两个 smoke 的暂停令牌断言同步为红、FRONTEND_SPEC 改写为 Aurora Ink 规范、新增 `scripts/review` 视觉验收驱动；全部 232 项自动化测试与 format/typecheck/lint/build 门禁保持通过。
