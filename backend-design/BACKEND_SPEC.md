...
2. 番茄 To-do 客户端可用时，通过原生桥按会话批量上传。
3. 只有服务器/客户端返回真实云确认后才设置 `isSynced=1`。
...
- 学科更改和删除必须更新/清理对应本地与云记录，两个同步域互不冒充对方成功。

## 8. 小窗与边缘状态

- 只有 `collapsed` 和 `expanded` 两种合法尺寸，数值唯一来自 `shared/miniWindowLayout.ts`。v0.10 保持 `184×35` 与 `256×92`，本轮不引入第三尺寸；常量变化必须同步更新前端规范。
- collapsed renderer 契约仅允许进度/状态、当前时间和展开入口；不传达任务详情、三组累计或其他控制。验收字号为 collapsed 23.5px、expanded 30px。
- Electron 主进程持有真实 bounds、当前显示器 work area、吸附边缘和窗口状态。
- 拖动结束后才计算最近合法边缘；使用进入 14px / 离开 30px 双阈值，贴边稳定 260ms 后收起成进度条。
...
- 番茄变化：分别验证客户端关闭、本地待上传、客户端运行、云确认、学科修改和删除。
