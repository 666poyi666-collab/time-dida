...
// 用法：
//   阶段1（崩溃前 running）：npx electron dist-selftest/crash.cjs start
//   阶段2（恢复 running）：  npx electron dist-selftest/crash.cjs recover-running
//   阶段3（崩溃前 paused）：  npx electron dist-selftest/crash.cjs pause-crash
//   阶段4（恢复 paused）：    npx electron dist-selftest/crash.cjs recover-paused
import { app } from 'electron';
import fs from 'node:fs';
import path from 'node:path';
import { initDatabase, closeDatabase, listSessions, getActiveSession, listSegments, listPauses, getSession } from '../electron/db/index.js';
...
app.whenReady().then(async () => {
  ...
  try {
    if (phase === 'start') {
      // ===== 阶段1：start + 等2s + 崩溃（process.exit 模拟强杀，跳过 before-quit）=====
